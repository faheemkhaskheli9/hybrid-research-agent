...
def test_long_text_is_split_with_overlap():
    text = " ".join(f"word{i}" for i in range(200))  # long enough to force multiple chunks
    chunks = chunk_text(text, chunk_size=50, chunk_overlap=10)
    assert len(chunks) > 1
    assert all(len(c) <= 50 for c in chunks)
    # reconstructed text (ignoring overlap dupes) should cover the original words
...
def test_chunking_terminates_on_pathological_overlap():
    # chunk_overlap close to chunk_size must not cause an infinite loop
    text = "x" * 1000
